...
const FEATURES = [
  { emoji: "🧠", title: "Smart Online Quizzes",   color: "#2563eb", tags: ["MCQ","Auto-Eval","Real-time"],      desc: "Real-time MCQ exams with instant auto-evaluation. Zero manual checking — results appear the moment students submit." },
...
const S = {
  ...
  label:      { fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#2563eb", marginBottom: 8 },
  ...
  btnPrimary: { background: "#2563eb", color: "#fff", padding: "11px 26px", borderRadius: 10, fontWeight: 700, fontSize: "0.92rem", cursor: "pointer", border: "none", display: "inline-flex", alignItems: "center", gap: 8, transition: "background 0.2s, transform 0.15s" },
  btnOutline: { background: "transparent", color: "#2563eb", padding: "11px 26px", borderRadius: 10, fontWeight: 600, fontSize: "0.92rem", cursor: "pointer", border: "1.5px solid #2563eb", display: "inline-flex", alignItems: "center", gap: 8, transition: "all 0.2s" },
};
...
const GLOBAL_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  input:focus, textarea:focus, select:focus { border-color:#2563eb !important; box-shadow:0 0 0 3px rgba(37,99,235,0.12) !important; outline:none; }
  ...
  return (
    <>
      ...
      <nav style={{ ...S.nav, boxShadow: scrolled ? "0 2px 20px rgba(0,0,0,0.08)" : "none" }}>
        <span style={S.logo} onClick={() => go("home")}>
          Quiz<span style={{ color: "#2563eb" }}>Matrix</span>
        </span>
        ...
        <div className="qm-desktop-nav" style={{ display: "flex", alignItems: "center", gap: "2rem" }}>
          {navItems.map(({ label, action }) => (
            <span key={label} onClick={action} style={S.navLink}
              onMouseEnter={e => { e.target.style.color = "#2563eb"; e.target.style.borderBottomColor = "#2563eb"; }}
              onMouseLeave={e => { e.target.style.color = "#64748b"; e.target.style.borderBottomColor = "transparent"; }}>
              {label}
            </span>
          ))}
          <button className="qm-btn-p" style={S.btnPrimary} onClick={() => go("home")}>
            Get Started Free →
          </button>
        </div>
      ...
      {menuOpen && (
        <div className="qm-mobile-menu" style={{ position: "fixed", top: 64, left: 0, right: 0, background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "1rem 5%", zIndex: 99, display: "flex", flexDirection: "column", gap: "0.8rem" }}>
          ...
          <button className="qm-btn-p" style={{ ...S.btnPrimary, justifyContent: "center", marginTop: 4 }} onClick={() => { go("home"); setMenuOpen(false); }}>
            Get Started Free →
          </button>
        </div>
      )}
...
function StatCard({ stat }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: "center" }}>
      <div style={{ fontSize: "1.9rem", fontWeight: 800, color: "#2563eb", letterSpacing: "-0.04em", lineHeight: 1 }}>{display}</div>
    ...
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "7rem 5% 5rem", background: "linear-gradient(155deg,#f0f7ff 0%,#f8fafc 50%,#faf5ff 100%)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 760, margin: "0 auto" }}>
        ...
        <h1 style={{ fontWeight: 800, fontSize: "clamp(2.6rem,6vw,4.6rem)", lineHeight: 1.06, letterSpacing: "-0.035em", color: "#0f172a", marginBottom: 20, animation: "fadeUp 0.7s ease 0.1s both" }}>
          ...
          <span style={{ color: "#2563eb" }}>QuizMatrix</span>
        </h1>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", animation: "fadeUp 0.7s ease 0.3s both" }}>
          <button className="qm-btn-p" style={S.btnPrimary} onClick={() => go("home")}>🚀 Start For Free</button>
          <button className="qm-btn-o" style={S.btnOutline} onClick={() => go("mock-tests")}>📝 Try a Mock Test</button>
        </div>
  ...
  return (
    <section id="features" style={S.sectionW}>
      <div style={{ textAlign: "center", marginBottom: "3.5rem" }}>
        ...
        <h2 style={{ ...S.h2, textAlign: "center" }}>Everything You Need to <span style={{ color: "#2563eb" }}>Quiz Smarter</span></h2>
    ...
    <section id="how-it-works" style={S.section}>
      <div className="qm-how-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }}>
        <div ref={leftRef} className={`qm-slide-l${leftVis ? " vis" : ""}`}>
          ...
          <h2 style={S.h2}>Up & Running in <span style={{ color: "#2563eb" }}>3 Easy Steps</span></h2>
          ...
          {steps.map((s, i) => (
            <div key={s.n} style={{ display: "flex", gap: 16, paddingBottom: 28, position: "relative" }}>
              ...
              <div style={{ minWidth: 42, height: 42, borderRadius: 12, background: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: "0.85rem", flexShrink: 0 }}>
                {s.n}
              </div>
              <div>
        ...
        <div ref={rightRef} className={`qm-slide-r${rightVis ? " vis" : ""}`}>
          <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.10)" }}>
            ...
            <div style={{ padding: "1.4rem" }}>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "0.74rem", color: "#2563eb", background: "#eff6ff", padding: "3px 10px", borderRadius: 100, marginBottom: 14 }}>⏱ 08:42 remaining</div>
              ...
              {[["A","Queue",false],["B","Stack",true],["C","Linked List",false],["D","Tree",false]].map(([l, opt, sel]) => (
                <div key={l} style={{ background: sel ? "#eff6ff" : "#f8fafc", border: `1px solid ${sel ? "#bfdbfe" : "#e2e8f0"}`, borderRadius: 8, padding: "8px 12px", fontSize: "0.8rem", marginBottom: 6, color: sel ? "#2563eb" : "#475569", fontWeight: sel ? 600 : 400, display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ width: 20, height: 20, borderRadius: "50%", background: sel ? "#2563eb" : "#e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.68rem", color: sel ? "#fff" : "#64748b", fontWeight: 700, flexShrink: 0 }}>{l}</span>
                  {opt} {sel && "✓"}
                </div>
              ))}
              <div style={{ height: 5, background: "#f1f5f9", borderRadius: 3, marginTop: 14 }}>
                <div className="qm-progress-bar" style={{ height: "100%", background: "linear-gradient(90deg,#2563eb,#7c3aed)", borderRadius: 3 }} />
              </div>
  ...
  return (
    <section style={{ background: "linear-gradient(135deg,#eff6ff,#faf5ff)", borderTop: "1px solid #e2e8f0", padding: "5rem 5%", textAlign: "center" }}>
      <div ref={ref} className={`qm-reveal${vis ? " vis" : ""}`}>
        ...
        <h2 style={{ ...S.h2, textAlign: "center" }}>Ready to <span style={{ color: "#2563eb" }}>Transform</span> How You Learn?</h2>
...
/* ─── TESTIMONIAL CARD (own component) ───────────────────── */
function TestiCard({ t, delay }) {
  const [ref, vis] = useScrollReveal();
  return (
    <div ref={ref} className={`qm-reveal${vis ? " vis" : ""}`}
      style={{ ...S.card, transitionDelay: `${delay}s` }}>
      <div style={{ color: "#f59e0b", fontSize: "0.85rem", marginBottom: 10, letterSpacing: 2 }}>
        {"★".repeat(t.stars)}{"☆".repeat(5 - t.stars)}
      </div>
      <p style={{ color: "#475569", fontSize: "0.875rem", lineHeight: 1.7, fontStyle: "italic", marginBottom: 16 }}>"{t.text}"</p>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 38, height: 38, borderRadius: "50%", background: t.color, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: "0.8rem", flexShrink: 0 }}>{t.initials}</div>
        <div>
...
function ContactPage() {
  const [form, setForm] = useState({ fname: "", lname: "", email: "", role: "", subject: "", message: "" });
  const [sent, setSent]   = useState(false);

  const inp = { width: "100%", background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, padding: "10px 14px", color: "#0f172a", fontSize: "0.9rem", transition: "border-color 0.2s, box-shadow 0.2s", fontFamily: "'Plus Jakarta Sans',sans-serif" };
  const lbl = { fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "#64748b", display: "block", marginBottom: 5 };

  const handleSubmit = () => {
    if (!form.email || !form.message) { alert("Please fill in Email and Message."); return; }
    setSent(true);
    setTimeout(() => setSent(false), 5000);
    setForm({ fname: "", lname: "", email: "", role: "", subject: "", message: "" });
  };

  const contactDetails = [
    { icon: "📧", label: "Email Us",       val: "hello@quizmatrix.app"       },
  ...
  return (
    <div style={{ paddingTop: 80, minHeight: "100vh", background: "#f8fafc" }}>
      <div style={{ padding: "4rem 5% 2rem", textAlign: "center" }}>
        ...
        <h1 style={{ ...S.h2, textAlign: "center" }}>Contact <span style={{ color: "#2563eb" }}>QuizMatrix</span></h1>
        <p style={{ ...S.sub, margin: "0 auto" }}>Questions, feedback, or partnership enquiries — we're here to help.</p>
      </div>

      <div className="qm-contact-grid" style={{ maxWidth: 1000, margin: "0 auto", padding: "2rem 5% 5rem", display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: "2.5rem", alignItems: "start" }}>
        {/* Info */}
        <div>
          <h3 style={{ fontWeight: 700, fontSize: "1.15rem", color: "#0f172a", marginBottom: 12 }}>Let's Talk 👋</h3>
          <p style={{ color: "#64748b", fontSize: "0.875rem", lineHeight: 1.7, marginBottom: 20 }}>Whether you're a student, teacher, or institution — our team is ready to help you get the most out of QuizMatrix.</p>
          {contactDetails.map(d => (
            <div key={d.label} style={{ display: "flex", alignItems: "center", gap: 12, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: "12px 14px", marginBottom: 10 }}>
              <span style={{ fontSize: "1.1rem" }}>{d.icon}</span>
              <div>
                <div style={{ fontWeight: 600, fontSize: "0.82rem", color: "#0f172a" }}>{d.label}</div>
                <div style={{ fontSize: "0.82rem", color: "#64748b" }}>{d.val}</div>
              </div>
            </div>
          ))}
          <div style={{ marginTop: 20 }}>
            <p style={{ fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "#94a3b8", marginBottom: 10 }}>Follow Us</p>
            <div style={{ display: "flex", gap: 8 }}>
              {[["📸","Instagram"],["📘","Facebook"],["🐙","GitHub"],["💼","LinkedIn"]].map(([icon, name]) => (
                <button key={name} title={name} style={{ width: 38, height: 38, borderRadius: 10, background: "#fff", border: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "1rem", transition: "all 0.2s" }}>{icon}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "2rem" }}>
          <h3 style={{ fontWeight: 700, fontSize: "1.1rem", color: "#0f172a", marginBottom: 20 }}>Send Us a Message ✉️</h3>
          <div className="qm-form-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
            <div><label style={lbl}>First Name</label><input style={inp} value={form.fname} onChange={e => setForm({ ...form, fname: e.target.value })} placeholder="Rahul" /></div>
            <div><label style={lbl}>Last Name</label><input style={inp} value={form.lname} onChange={e => setForm({ ...form, lname: e.target.value })} placeholder="Kumar" /></div>
          </div>
          <div style={{ marginBottom: "1rem" }}>
            <label style={lbl}>Email Address</label>
            <input type="email" style={inp} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="you@example.com" />
          </div>
          <div style={{ marginBottom: "1rem" }}>
            <label style={lbl}>I Am A</label>
            <select style={inp} value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}>
              <option value="">Select role...</option>
              <option>Student</option><option>Teacher / Educator</option><option>School / Institution</option><option>Other</option>
            </select>
          </div>
          <div style={{ marginBottom: "1rem" }}>
            <label style={lbl}>Subject</label>
            <input style={inp} value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} placeholder="How can we help?" />
          </div>
          <div style={{ marginBottom: "1.2rem" }}>
            <label style={lbl}>Message</label>
            <textarea style={{ ...inp, minHeight: 110, resize: "vertical" }} value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} placeholder="Tell us more..." />
          </div>
          <button className="qm-btn-p" style={{ ...S.btnPrimary, width: "100%", justifyContent: "center" }} onClick={handleSubmit}>
            Send Message 🚀
          </button>
          {sent && (
            <div style={{ marginTop: 12, background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: "10px 14px", color: "#059669", fontSize: "0.875rem", textAlign: "center" }}>
    ...
    <footer style={{ background: "#0f172a", color: "#94a3b8", padding: "4rem 5% 2rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div className="qm-footer-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: "2.5rem", marginBottom: "2.5rem" }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: "1.4rem", color: "#fff", marginBottom: 12, letterSpacing: "-0.03em", cursor: "pointer" }} onClick={() => go("home")}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[["📸","Instagram"],["📘","Facebook"],["🐙","GitHub"],["💼","LinkedIn"]].map(([icon, name]) => (
                <button key={name} title={name} style={{ width: 36, height: 36, borderRadius: 9, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "0.9rem", color: "#94a3b8", transition: "all 0.2s" }}>{icon}</button>
              ))}
            </div>
          </div>
          {cols.map(col => (
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <p style={{ fontSize: "0.8rem" }}>© 2025 QuizMatrix. All rights reserved. Built with ❤️ for learners.</p>
